...
int calculate_age(const char* dob) {
    ...
    if ((current->tm_mon + 1 < month) || 
        (current->tm_mon + 1 == month && current->tm_mday < day)) {
        age--;
    }
...
void export_card(struct student s)
{
    ...
    fprintf(card_fp, "DATE OF BIRTH  : %s\n", s.dob);
    fprintf(card_fp, "AGE            : %d\n", s.age);
...
void register_a_form()
{
    ...
    {
        char dummy_name[50], dummy_id[15], dummy_bg[5], dummy_allergies[50], dummy_medical[100],dummy_dob;
        int dummy_age;
        sscanf(line, "%49[^,],%14[^,],%10[^,],%d,%4[^,],%49[^,],%99[^,],%99[^\n]", dummy_name, dummy_id,dummy_dob,dummy_age, dummy_bg, dummy_allergies, dummy_medical, file_mail_id);
    ...
    fprintf(fp, "%s,%s,%s,%d,%s,%s,%s,%s\n", s.name, s.student_id,s.dob,s.age, s.blood_group, s.allergies, s.medical_issue, s.mail_id);
...
void staff(char id[20])
{
    ...
    while(fgets(line,sizeof(line),fp)!=NULL)
    {
        ...
        fprintf(tempfp,"%s,%s,%s,%d,%s,%s,%s,%s\n",s.name,s.student_id,s.dob,s.age,s.blood_group,s.allergies,s.medical_issue,s.mail_id);
